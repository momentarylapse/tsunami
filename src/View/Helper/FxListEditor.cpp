...
FxListEditor::FxListEditor(AudioViewTrack *t, hui::Panel *p, const string &_id, const string &_id_midi) {
	panel = p;
	id_fx_list = _id;
	id_midi_fx_list = _id_midi;
	vtrack = t;
	assert(vtrack);
	panel->event_x(id_fx_list, "hui:select", [=]{ on_fx_select(); });
	panel->event_x(id_fx_list, "hui:change", [=]{ on_fx_edit(); });
	panel->event_x(id_fx_list, "hui:move", [=]{ on_fx_move(); });
	panel->event_x(id_fx_list, "hui:right-button-down", [=]{ on_fx_right_click(); });
	panel->event("add-fx", [=]{ on_add_fx(); });
	panel->event("fx-add", [=]{ on_add_fx(); });
	panel->event("fx-delete", [=]{ on_fx_delete(); });
	panel->event("fx-enabled", [=]{ on_fx_enabled(); });
	panel->event("fx-copy-from-track", [=]{ on_fx_copy_from_track(); });
	panel->event_x(id_midi_fx_list, "hui:select", [=]{ on_midi_fx_select(); });
	panel->event_x(id_midi_fx_list, "hui:change", [=]{ on_midi_fx_edit(); });
	panel->event_x(id_midi_fx_list, "hui:move", [=]{ on_midi_fx_move(); });
	panel->event("add-midi-fx", [=]{ on_add_midi_fx(); });
...
Session *FxListEditor::session() const {
	if (!vtrack)
		return nullptr;
	return vtrack->track->song->session;
}

Track *FxListEditor::track() const {
	if (!vtrack)
		return nullptr;
	return vtrack->track;
}
