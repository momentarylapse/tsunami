...
static const complex exp_16[8] = {
		complex(1, 0),
		complex(cos(pi/8), -sin(pi/8)),
		complex(cos(pi/4), -sin(pi/4)),
		complex(cos(pi/8*3), -sin(pi/8*3)),
		complex(0, -1),
		complex(cos(pi/8*5), -sin(pi/8*5)),
		complex(cos(pi/8*6), -sin(pi/8*6)),
		complex(cos(pi/8*7), -sin(pi/8*7))
};
static Array<complex*> exp_2n;
...
void fft_c2c_2n(complex *in, complex *out, int n, int n2, int stride)
{
	complex *o = fft_temp_out[n];
	complex *exp_const = exp_2n[n];
	/*if (n2 == 8){
		fft_c2c_4(in, out, stride * 2);
		fft_c2c_4(&in[stride], o, stride * 2);
	}else*/ if (n2 == 16){
		fft_c2c_8(in, out, stride * 2);
		fft_c2c_8(&in[stride], o, stride * 2);
	}else{
		fft_c2c_2n(in, out, n - 1, n2 >> 1, stride * 2);
		fft_c2c_2n(&in[stride], o, n - 1, n2 >> 1, stride * 2);
	}
	int n22 = n2/2;
	out[n22] = out[0] - o[0];
	for (int i=1;i<n22;i++)
