...
Temperament Temperament::create_default() {
	Temperament t;
	for (int p=0; p<MAX_PITCH; p++)
		t.freq[p] = pitch_to_freq((float)p);
	return t;
}

Temperament Temperament::create(TemperamentType type, int pitch_ref, float freq_ref) {
	auto create_from_factor = [pitch_ref, freq_ref] (float *factor) {
		Temperament t;
	...
	auto create_meantone = [create_from_factor] (float x) {
		...
		return create_from_factor(factor);
	};
