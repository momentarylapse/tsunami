	...
	hui::Window(AppName, 800, 600)
{
	...
	event("add-track-audio-mono", [=]{ on_add_audio_track_mono(); });
	set_key_code("add-track-audio-mono", -1, "hui:add");
	event("add-track-audio-stereo", [=]{ on_add_audio_track_stereo(); });
	set_key_code("add-track-audio-stereo", -1, "hui:add");
	event("add-track-group", [=]{ song->add_track(SignalType::GROUP); });
	event("add-track-beats", [=]{ on_add_time_track(); });
	set_key_code("add-track-beats", -1, "hui:add");
	event("add-track-midi", [=]{ on_add_midi_track(); });
	set_key_code("add-track-midi", -1, "hui:add");
	event("delete-track", [=]{ on_delete_track(); });
	set_key_code("delete-track", -1, "hui:delete");
