...
void DynamicArray::append_single(const void *d) {
	simple_reserve(num + 1);
	memcpy(simple_element(num), d, element_size);
	num ++;
}
...
void DynamicArray::insert_single(const void *d, int index) {
	insert_blank(index);
	memcpy(simple_element(index), d, element_size);
}
...
void DynamicArray::simple_swap(int i1, int i2) {
	...
	if (i1 == i2)
		return;
	char *p1 = (char*)simple_element(i1);
	char *p2 = (char*)simple_element(i2);
	if (element_size == 1) {
		char t = *p1;
		*p1 = *p2;
		*p2 = t;
...
#else
	memmove(simple_element(index), simple_element(index + 1), (num - index - 1) * element_size);
#endif
	num --;
}
...
int DynamicArray::simple_index(const void *p) const {
	return ((int_p)p - (int_p)data) / element_size;
}

void* DynamicArray::simple_element(int index) {
