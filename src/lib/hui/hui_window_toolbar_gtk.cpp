...
void HuiWindow::EnableToolbar(bool enabled)
{
	if (enabled)
...
void HuiWindow::ToolbarConfigure(bool text_enabled, bool large_icons)
{
...
{	NotifyWindowByWidget((HuiWindow*)data, widget);	}
...
void HuiWindow::ToolbarAddItemMenu(const string &title, const string &tool_tip, const string &image, HuiMenu *menu, const string &id)
{
	if (!menu)
		return;
	GtkWidget *im=(GtkWidget*)get_gtk_image(image,true);
...
void HuiWindow::_ToolbarEnable_(const string &id, bool enabled)
{
	allow_signal_level ++;
	for (int t=0;t<4;t++)
...
void HuiWindow::_ToolbarCheck_(const string &id, bool checked)
{
	allow_signal_level++;
	for (int t=0;t<4;t++)
		foreach(HuiToolbarItem &it, toolbar[t].item)
...
bool HuiWindow::_ToolbarIsChecked_(const string &id)
{
	for (int t=0;t<4;t++)
		foreach(HuiToolbarItem &it, toolbar[t].item)
			if (id == it.id)
