...
//	add_cmd(, - d->cur_func->_VarSize - LocalOffset - 8);
	...
	//dp += 8; // esp, eip
	if (push_size > 127)
		add_cmd(d, Asm::inst_add, param_reg(TypePointer, Asm::RegEsp), param_const(TypeInt, (void*)push_size));
	else if (push_size > 0)
		add_cmd(d, Asm::inst_add_b, param_reg(TypePointer, Asm::RegEsp), param_const(TypeInt, (void*)push_size));
...
// do all the translations...
	...
	//if (call_used){
	d->StackMaxSize = ((d->StackMaxSize + StackMemAlign - 1) / StackMemAlign) * StackMemAlign;
